...
- **`hydrate(loader)` — fill a `data` store once from a loader, then own it.** Sits on top of `ctx.forward()`/`data`. A `data` endpoint whose loader is wrapped in `hydrate()` runs the loader **once** on first access, sets the result into the store, latches `hydrated=true`, and serves the store thereafter — so default CRUD (POST/PUT/PATCH/DELETE) mutations **stick** over the captured snapshot. An **unwrapped** loader runs **every** request (live; mutations clobbered). Loader signature `(req, ctx) => T | Promise<T>` with full `HandlerContext` (incl. `forward`) — proxy-agnostic (loader may read a file/inline instead). Surfaces: declarative `.data(url, hydrate(loader))` (common case); primitive `ctx.endpoint(url).hydrate(loader)` and current-endpoint `ctx.hydrate(loader)` (advanced/mixed handlers — hydrate a single call inside a larger handler). `hydrated` latches on **any** write (loader OR CRUD), so a mutation before first load is preserved (loader skipped). `handle.reset()` clears the store + un-latches → next access re-loads. Concurrent first accesses share one in-flight promise (loader runs once). Distinct from **Mem-session** (global, mode-gated record/replay cache, immutable) — `hydrate` produces owned, CRUD-mutable endpoint data, latched per-endpoint until `reset()`. No bare `hydrate()` — loader always explicit.
