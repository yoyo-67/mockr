...
A WS endpoint is a plain def in `endpoints` — the builder doesn't emit WebSockets, so author it directly. Instead of `data` / `dataFile` / a handler, attach `ws: ws({...})`. The factory takes lifecycle hooks (`onConnect`, `onMessage`, `onClose`) plus optional `message` / `query` / `params` zod schemas. HTTP routes still go through `mockGroup()` and ride alongside in `groups`.
...
```ts
import { mockr, mockGroup, ws, type WsEndpoint } from '@yoyo-org/mockr';
import { z } from 'zod';
...
mockr<Endpoints>({
  port: 3011,
  groups: [http],
  // WebSocket endpoints are authored as plain defs.
  endpoints: [
    {
      url: '/ws/agent',
...
[**Open in StackBlitz →**](https://stackblitz.com/github/yoyo-67/mockr?file=examples/11-ai-agent-ws/server.ts) — paste each command into the StackBlitz Terminal once `npx tsx examples/11-ai-agent-ws/server.ts` is running.
